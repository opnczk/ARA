...
public  class ApplicativeProtocol implements EDProtocol{
	...
	private boolean shouldBroadcast(Node host) {
		//On ne peut broadcast qu'une fois.
		if(broadcast_done){
			return false;
		}
		...
		if(!nbMessReceived.containsKey(winner.getID())){
			//Si on n'a jamais recu de message du winner
			return false;
		}
		if(nbMessReceived.get(winner.getID()) < 2  ){
			// si on en a recu un seul
			return false;
		}
		
		//On broadcast lorsque tout les noeuds créés avant nous on déjà broadcast deux fois au moins, et qu'ils sont fiables
		return true;
	}
